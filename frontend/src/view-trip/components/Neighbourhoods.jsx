...
export default function Neighbourhoods({ trip }) {
  const items = coalesceArray(trip, [
    "tripData.neighbourhoods",
    "neighbourhoods",
    "tripData.neighborhoods",
    "neighborhoods",
    "tripData.areas",
    "areas",
    "tripData.districts",
  ...
  return (
    <div>
      ...
      <div
        ...
        aria-label="Neighbourhoods carousel"
        role="region"
        ...
        {items.map((raw, idx) => {
          ...
          return (
            <article key={key} className="group relative rounded-2xl border bg-card hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col snap-center">
              ...
              <SmartImage
                  query={`${title} neighbourhood area ${location}`}
                  alt={title}
